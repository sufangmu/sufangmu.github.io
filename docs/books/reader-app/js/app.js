...
(function () {
  ...
  function categoryLabel(id) {
    var labels = { 'leo_nce_notes': 'leo新概念英语笔记', 'novel': '小说' };
    return labels[id] || id;
  }
  ...
  function loadBook(book) {
    ...
    if (book.format === 'epub') loadEpub(path);
    else if (book.format === 'pdf') loadPdf(path);
    else if (book.format === 'mobi' || book.format === 'azw3') loadMobi(book);
    else showEmpty();
  }
  ...
  function loadEpub(path) {
    ...
    showProgress('正在下载 EPUB …');

    xhrDownload(path, function (pct) {
      setProgress(Math.round(pct * 50));
    }).then(function () {
      setProgressMsg(52, '正在准备容器 …');
      ...
      var pulseTimer = setInterval(function () {
        var cur = parseInt(dom.loadingFill.style.width) || 75;
        if (cur < 95) {
  ...
  // 顺序渲染到指定页码（自动补齐中间未渲染的页）
  function renderPdfUpTo(target) {
    state.pdfRendered.sort(function (a, b) { return a - b; });
    var highest = state.pdfRendered[state.pdfRendered.length - 1] || 0;
    if (target <= highest) return Promise.resolve();

    function renderNext(p) {
      if (p > target) return Promise.resolve();
      return renderPdfPage(p).then(function () { return renderNext(p + 1); });
    }
    return renderNext(highest + 1);
  }
  ...
  //  MOBI 加载
  // ==========================================================================
  function loadMobi(book) {
    showProgress('正在下载 MOBI …');
    xhrDownload(book.path, function (pct) {
      setProgress(Math.round(pct * 85));
    }).then(function (buffer) {
      setProgressMsg(95, '正在提取文本 …');
      setTimeout(function () {
        dom.epubView.style.display = 'none';
  ...
  function adjustZoom(dir) {
    if (state.currentFormat === 'pdf' && state.pdfDoc) {
      ...
      dom.pdfView.innerHTML = '';
      state.pdfRendered = [];
      renderPdfUpTo(currentPage).then(function () {
        setTimeout(function () { smoothScrollToPage(currentPage); }, 50);
      });
    ...
    dom.progressSlider.addEventListener('input', function () {
      if (state.currentFormat === 'epub' && state.epubRendition && state.epubBook) {
        ...
        if (pageNum >= 1 && pageNum <= state.pdfTotal) {
          if (state.pdfRendered.indexOf(pageNum) !== -1) {
            smoothScrollToPage(pageNum);
            state.pdfPage = pageNum;
            updatePdfProgress();
          } else {
            state.pdfPage = pageNum;
            renderPdfUpTo(pageNum).then(function () {
              setTimeout(function () { smoothScrollToPage(pageNum); }, 50);
            });
          }
        }
      }
  ...
  function navigatePage(dir) {
    if (state.currentFormat === 'epub' && state.epubRendition) {
      ...
      if (dir > 0) {
        // 向前翻：如果页已渲染直接平滑滚动，否则渲染后滚动
        if (state.pdfRendered.indexOf(newPage) !== -1) {
          smoothScrollToPage(newPage);
          state.pdfPage = newPage;
          updatePdfProgress();
        } else {
          renderPdfPage(newPage).then(function () {
            setTimeout(function () { smoothScrollToPage(newPage); }, 50);
          });
        }
      } else {
        // 向后翻：平滑滚动到上一页
