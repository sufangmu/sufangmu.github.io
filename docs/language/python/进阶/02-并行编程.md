...
运行中的程序。每个进行拥有自己的地址空间、内存、数据栈等。多进程之间的数据不共享。进程也可以通过派生（fork 或 spawn）新的进程来执行其他任务，进程间采用进程间通信（IPC，Inter Process communication）的方式共享信息。
...
def search_ticket():
    with open(r"E:\notes\testcode\ticket.db", 'r', encoding='utf-8') as f:
        ticket = json.load(f)
        print('当前余票：%d张.' % ticket['num'],end='')
...
def buy_ticket(i):
    ...
    if ticket['num'] > 0:
        ...
        print('余票不足，购票失败!')
    time.sleep(0.1)
    with open(r'E:\notes\testcode\ticket.db', mode='w', encoding='utf-8') as f:
        json.dump(ticket, f)
